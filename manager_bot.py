...
# === Настройки ===
TELEGRAM_TOKEN = "8773186067:AAFEtMtaKtkTTGH6HLNmWEzdXHlFKYh4g3g"  # ← Новый токен менеджера
...
async def process_username(message: Message, state: FSMContext):
    ...
    await message.answer("Telegram User ID клиента (число, например 566752574):")
    await state.set_state(ManagerForm.telegram_user_id)

@dp.message(ManagerForm.telegram_user_id)
async def process_user_id(message: Message, state: FSMContext):
    await state.update_data(telegram_user_id=message.text.strip())
...
async def process_start_date(message: Message, state: FSMContext):
    ...
    data['start_date'] = message.text.strip()

    # Создание контакта
    contact_data = {
        "contact": {
            "first_name": data['fio'].split()[0],
            "last_name": ' '.join(data['fio'].split()[1:]),
            "phone": data['phone'],
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f"{OKDESK_API_BASE}/contacts", json=contact_data, params={"api_token": OKDESK_API_TOKEN}) as resp:
            if resp.status != 201:
    ...
    entity_data = {
        "maintenance_entity": {
            ...
            "custom_fields": {
                "free_service_start_date": data['start_date']
            },
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f"{OKDESK_API_BASE}/maintenance_entities", json=entity_data, params={"api_token": OKDESK_API_TOKEN}) as resp:
            if resp.status != 201:
                text = await resp.text()
...
async def process_user_id(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Пожалуйста, введите число (User ID).")
